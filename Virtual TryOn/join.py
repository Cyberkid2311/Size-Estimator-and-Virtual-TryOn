...
from customer import userPreprocess
...
class userFit(userPreprocess, catPreprocess):
    ...
    def colorUser(self):
        ...
        for i in range(grayUserImg.shape[0]):
            for j in range(grayUserImg.shape[1]):
                if(visited[i][j] == 0):
    ...
    def resizeCat(self):
        ...
        self.catImg = cv2.resize(
            self.catImg, (int(self.userImg.shape[1]), int(self.userImg.shape[0])))
        ...
        for j in range(grayCatImg.shape[1]):
            if grayCatImg[grayCatImg.shape[0]-100][j] != 0:
                pixCountCat += 1

        self.ratio += float(pixCountCat-pixCountUser) / \
            float(grayUserImg.shape[1])
        # print self.ratio
        self.catImg = cv2.resize(self.catImg, (int(self.ratio*self.userImg.shape[1]), int(
            self.ratio*self.userImg.shape[0])), interpolation=cv2.INTER_NEAREST)
        self.leftArmCat = cv2.resize(self.leftArmCat, (int(self.ratio*self.userImg.shape[1]), int(
            self.ratio*self.userImg.shape[0])), interpolation=cv2.INTER_NEAREST)
        self.rightArmCat = cv2.resize(self.rightArmCat, (int(
            self.ratio*self.userImg.shape[1]), int(self.ratio*self.userImg.shape[0])), interpolation=cv2.INTER_NEAREST)
    ...
    def bodyFit(self, colorUserImg):
        ...
        for i in range(self.catImg.shape[0]):
        ...
        for i in range(0, grayUserImg.shape[0]):
            if i + self.catShLine - self.userShLine >= self.catImg.shape[0]:
                break
            for j in range(self.userMid, grayUserImg.shape[1]):
                if self.catMid + j - self.userMid < self.catImg.shape[1]:
                    if grayUserImg[i][j] != grayUserImg[0][0] and (self.catImg[i + self.catShLine - self.userShLine][self.catMid + j - self.userMid] != self.catImg[0][0]).any():
                        bodyFitOut[i][j] = self.catImg[i + self.catShLine -
                                                       self.userShLine][self.catMid + j - self.userMid]

        for i in range(0, grayUserImg.shape[0]):
            if i + self.catShLine - self.userShLine >= self.catImg.shape[0]:
                break
            for j in range(self.userMid, -1, -1):
	...
	def sleeveFit(self,bodyFitOut):
		...
		startCat = []
		endCat = []
		for j in range(0, bodyFitOut.shape[1]):
			for i in range(0, bodyFitOut.shape[0]):
				if bodyFitOut[i][j][0] != bodyFitOut[0][0][0] or bodyFitOut[i][j][1] != bodyFitOut[0][0][1] or bodyFitOut[i][j][2] != bodyFitOut[0][0][2]:
					startUser.append(i)
					break

		for j in range(0, bodyFitOut.shape[1]):
			for i in range(startUser[j], bodyFitOut.shape[0]):
		...
		for j in range(0, self.catImgSleeve.shape[1]):
			for i in range(0, self.catImgSleeve.shape[0]):
		...
		for j in range(0, self.catImgSleeve.shape[1]):
			for i in range(startCat[j], self.catImgSleeve.shape[0]):
		...
		for j in range(0, bodyFitOut.shape[1]):
			if(j <= self.leftUserLine or j >= self.rightUserLine):
				for i in range((startUser[j]+endUser[j])/2,startUser[j]-1,-1):
				...
				for i in range((startUser[j]+endUser[j])/2,endUser[j]+1):
    ...
    def getBox(self, img):
        ...
        for i in range(grayImg.shape[0]-1, -1, -1):
            for j in range(grayImg.shape[1]):
        ...
        for j in range(grayImg.shape[1]-1, -1, -1):
            for i in range(grayImg.shape[0]):
    ...
    def rotateImage(self, img, theta, mid1, mid2):
        ...
        dst = cv2.warpAffine(
            img, M, (img.shape[1], img.shape[0]), flags=cv2.INTER_NEAREST)
        extra = 200
    ...
    def leftFit(self, userImg, catImg):
        ...
        check = 0
        for i in range(grayUser.shape[0]):
            for j in range(grayUser.shape[1]):
        ...
        check = 0
        for i in range(grayCat.shape[0]):
            for j in range(grayCat.shape[1]):
        ...
        for i in range(self.userShLine, grayUser.shape[0]):
            check = 0
            prev = 0
            for j in range(grayUser.shape[1]):
        ...
        for i in range(self.catShLine, grayCat.shape[0]):
            check = 0
            prev = 0
            for j in range(grayCat.shape[1]):
        ...
        for i in range(self.userShLine, userImg.shape[0]-1):
            for j in range(startUser[i], endUser[i]):
                try:
                    if grayCat[i + self.catShLine - self.userShLine][startCat[i + self.catShLine - self.userShLine] + j - startUser[i]] != 0:
                        output[i][j] = catImg[i + self.catShLine - self.userShLine][startCat[i +
                                                                                             self.catShLine - self.userShLine] + j - startUser[i]]
                except:
                    pass
    ...
    def rightFit(self, userImg, catImg):
        ...
        check = 0
        for i in range(grayUser.shape[0]):
            for j in range(grayUser.shape[1]-1, -1, -1):
                if grayUser[i][j] != grayUser[0][0]:
                    self.userShLine = i
                    check = 1
                    break
        ...
        check = 0
        for i in range(grayCat.shape[0]):
            for j in range(grayCat.shape[1]-1, -1, -1):
                if grayCat[i][j] != grayCat[0][0]:
                    self.catShLine = i
                    check = 1
                    break
        ...
        output = np.zeros(userImg.shape)
        for i in range(userImg.shape[0]):
            for j in range(startUser[i]-1, endUser[i], -1):
                try:
                    if grayCat[i + self.catShLine - self.userShLine][startCat[i + self.catShLine - self.userShLine] + j - startUser[i]] != 0:
                        output[i][j] = catImg[i + self.catShLine - self.userShLine][startCat[i +
                                                                                             self.catShLine - self.userShLine] + j - startUser[i]]
    ...
    def sleeveFit(self):
        self.resizeCat()
        extra = 200
        leftUser = np.lib.pad(self.leftArmUser, ((
            extra, extra), (extra, extra), (0, 0)), 'constant', constant_values=(0))
        rightUser = np.lib.pad(self.rightArmUser, ((
            extra, extra), (extra, extra), (0, 0)), 'constant', constant_values=(0))
        leftCat = np.lib.pad(self.leftArmCat, ((
            extra, extra), (extra, extra), (0, 0)), 'constant', constant_values=(0))
        rightCat = np.lib.pad(self.rightArmCat, ((
            extra, extra), (extra, extra), (0, 0)), 'constant', constant_values=(0))

        leftUserRot, leftUserX, leftUserY, leftUserTheta = self.rotateNSave(
            leftUser, 'left')
        rightUserRot, rightUserX, rightUserY, rightUserTheta = self.rotateNSave(
            rightUser, 'right')
        leftCatRot, leftCatX, leftCatY, leftCatTheta = self.rotateNSave(
            leftCat, 'left')
        rightCatRot, rightCatX, rightCatY, rightCatTheta = self.rotateNSave(
            rightCat, 'right')
        ...
        finalLeft = self.rotateImage(
            leftUserFit, -leftUserTheta, leftUserX, leftUserY)
        finalRight = self.rotateImage(
            rightUserFit, -rightUserTheta, rightUserX, rightUserY)

        return finalLeft, finalRight

    def finalFit(self, bodyFitOut, finalLeft, finalRight):
        for i in range(bodyFitOut.shape[0]):
            for j in range(bodyFitOut.shape[1]):
